...
// ddmmyyyy
// Actually i cannot understand the pola from purchashed_at, so i assume that purchashed_at's pola is ddmmyyyy and 2 last number of purchase code
                        ...
                        override fun onResponse(list: JSONArray?) {
                            if (list != null) {
                                for (i in 0 until list.length()) {
                                    ...
                                    if (date.subSequence(0, 8) == dateQuery.subSequence(0, 8)) {
                                        val tags = getList.getJSONArray("tags")
                                        tempList.tags.addAll(tags)
                                        tempList.inventoryId = getList.getString("inventory_id")
                                        tempList.name = getList.getString("name")
